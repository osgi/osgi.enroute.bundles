package osgi.enroute.web.server.provider;

import java.io.*;
import java.util.*;
import java.util.concurrent.locks.*;

import javax.servlet.*;
import javax.servlet.http.*;

import org.osgi.service.log.*;

import osgi.enroute.rootservlet.api2.*;

public class ConditionalServletController extends HttpServlet {
	private static final long 				serialVersionUID = 1L;
	private ReentrantLock					lock = new ReentrantLock();
	private Map<Integer,ConditionalServlet>	servlets = new TreeMap<>();
	private final LogService				logger;

	public ConditionalServletController(LogService logger) {
		this.logger = logger;
	}

	@Override
	protected void service(HttpServletRequest rq, HttpServletResponse rsp) throws ServletException, IOException {
		boolean isProcessed = false;
		try {
			lock.lock();
			Iterator<ConditionalServlet> i = servlets.values().iterator();
			while (i.hasNext() && !isProcessed) {
				ConditionalServlet servlet = i.next();
				try {
					isProcessed = servlet.doConditionalService(rq, rsp);					
				}
				catch (Exception e) {
					logger.log(LogService.LOG_ERROR, "A RootServlet threw an Exception and will be removed from the queue");
					unbindServlet(servlet);
				}
			}
		}
		finally {
			lock.unlock();
		}

		// TODO: What happens if NONE process the request? What should the default behaviour be??
	}

	void bindServlet(ConditionalServlet servlet, Map<String,Object> properties) {
		Integer ranking = (Integer)properties.get("service.ranking");
		if(ranking == null || ranking < 0) {
			logger.log(LogService.LOG_ERROR, "Bad ranking value");
		}
		try {
			lock.lock();
			servlets.put(ranking, servlet);
		}
		finally {
			lock.unlock();
		}
		
	}

	void unbindServlet(ConditionalServlet servlet) {
		try {
			lock.lock();
			servlets.values().remove(servlet);
		}
		finally {
			lock.unlock();
		}
	}

	Collection<ConditionalServlet> servlets()
	{
		return Collections.unmodifiableCollection(servlets.values());
	}
}
